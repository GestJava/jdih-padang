...
class UserModel extends \App\Models\BaseModel
{
	...
	{
		$sql = 'SELECT id, nama_instansi FROM instansi ORDER BY nama_instansi ASC';
		return $this->db->query($sql)->getResultArray();
	}

	public function saveData($user_permission = [])
	{
		$fields = ['nama', 'email', 'id_instansi'];
		if (in_array('update_all', $user_permission)) {
			...
			// Untuk user tanpa update_all permission, tetap ambil username karena required
			$fields[] = 'username';
		}
		...
		// Normalisasi FK agar tidak melanggar constraint
		// id_instansi: kosong/0 => NULL; selain itu pastikan ada di tabel instansi
		if (array_key_exists('id_instansi', $data_db)) {
			$idInstansi = $data_db['id_instansi'];
			// Convert ke string untuk validasi
			if (is_numeric($idInstansi)) {
				$idInstansi = (string) $idInstansi;
			} else {
				$idInstansi = trim((string) ($idInstansi ?? ''));
			}
			
			if ($idInstansi === '' || $idInstansi === '0' || $idInstansi === 'null') {
				// Hapus key dari array agar CodeIgniter set ke NULL dengan benar
				unset($data_db['id_instansi']);
				// Set explicit NULL jika field required
				$data_db['id_instansi'] = null;
			} else {
				$instansi = $this->db->table('instansi')->select('id')->where('id', (int) $idInstansi)->get()->getRowArray();
				if (!$instansi) {
					// Instansi tidak ditemukan, set ke NULL
					unset($data_db['id_instansi']);
					$data_db['id_instansi'] = null;
				} else {
					$data_db['id_instansi'] = (int) $idInstansi;
				}
			}
		}

		// default_page_id_role: kosong/0/invalid => NULL
		if (in_array('update_all', $user_permission) && array_key_exists('default_page_id_role', $data_db)) {
			$roleId = trim((string) ($data_db['default_page_id_role'] ?? ''));
			if ($roleId === '' || $roleId === '0') {
				$data_db['default_page_id_role'] = null;
			} else {
				$role = $this->db->table('role')->select('id_role')->where('id_role', (int) $roleId)->get()->getRowArray();
				if (!$role) {
					$data_db['default_page_id_role'] = null;
				}
			}
		...
		// default_page_id_module: kosong/0/invalid => NULL
		if (in_array('update_all', $user_permission) && array_key_exists('default_page_id_module', $data_db)) {
			$moduleId = trim((string) ($data_db['default_page_id_module'] ?? ''));
			if ($moduleId === '' || $moduleId === '0') {
				$data_db['default_page_id_module'] = null;
			} else {
				$module = $this->db->table('module')->select('id_module')->where('id_module', (int) $moduleId)->get()->getRowArray();
				if (!$module) {
					$data_db['default_page_id_module'] = null;
				}
			}
		...
		if ($this->request->getPost('id')) {
			...
			$update_result = $this->db->table('user')->update($data_db, ['id_user' => $id_user]);
			if (!$update_result) {
				$error = $this->db->error();
				$this->db->transRollback();
				$error_msg = is_array($error) && isset($error['message']) ? $error['message'] : 'Unknown database error';
				return ['status' => 'error', 'message' => 'Gagal mengupdate data user: ' . $error_msg];
			}
		} else {
			// User baru - pastikan field required ada
			if (empty($data_db['nama']) || empty($data_db['email']) || empty($data_db['username'])) {
				$this->db->transRollback();
				return ['status' => 'error', 'message' => 'Field nama, email, dan username harus diisi'];
			}
			
			// Pastikan password ada untuk user baru
			if (empty($data_db['password'])) {
				$password = $this->request->getPost('password');
				if (empty($password)) {
					$this->db->transRollback();
					return ['status' => 'error', 'message' => 'Password harus diisi'];
				}
				$data_db['password'] = password_hash($password, PASSWORD_DEFAULT);
			}
			
			// Set default values untuk user baru jika tidak ada permission update_all
			if (!in_array('update_all', $user_permission)) {
				if (empty($data_db['status'])) {
					$data_db['status'] = 'active';
				}
				if (!isset($data_db['verified'])) {
					$data_db['verified'] = 1;
				}
			}
			
			$insert_result = $this->db->table('user')->insert($data_db);
			if (!$insert_result) {
				$error = $this->db->error();
				$this->db->transRollback();
				$error_msg = is_array($error) && isset($error['message']) ? $error['message'] : (is_string($error) ? $error : 'Unknown error');
				return ['status' => 'error', 'message' => 'Gagal menyimpan data user: ' . $error_msg];
			}
			
			$id_user = $this->db->insertID();
			if (empty($id_user)) {
				$this->db->transRollback();
				return ['status' => 'error', 'message' => 'Gagal mendapatkan ID user yang baru dibuat'];
			}
			
			// Pastikan user benar-benar ada di database sebelum insert role
			$check_user = $this->db->table('user')->select('id_user')->where('id_user', $id_user)->get()->getRowArray();
			if (!$check_user) {
				$this->db->transRollback();
				return ['status' => 'error', 'message' => 'User tidak ditemukan setelah insert, kemungkinan ada constraint violation'];
			}
		}

		if (in_array('update_all', $user_permission)) {
			$data_db = [];
			$id_role_post = isset($_POST['id_role']) && is_array($_POST['id_role']) ? $_POST['id_role'] : [];
			foreach ($id_role_post as $id_role) {
				$rid = (int) $id_role;
				if ($rid <= 0) continue;
				$exists = $this->db->table('role')->select('id_role')->where('id_role', $rid)->get()->getRowArray();
				if ($exists) {
					$data_db[] = ['id_user' => $id_user, 'id_role' => $rid];
				}
			}

			// Hanya delete dan insert role jika user sudah ada (untuk edit)
			if ($this->request->getPost('id')) {
				$this->db->table('user_role')->delete(['id_user' => $id_user]);
			}
			
			if ($data_db) {
				$insert_role_result = $this->db->table('user_role')->insertBatch($data_db);
				if (!$insert_role_result) {
					$error = $this->db->error();
					$this->db->transRollback();
					$error_msg = is_array($error) && isset($error['message']) ? $error['message'] : (is_string($error) ? $error : 'Unknown error');
					return ['status' => 'error', 'message' => 'Gagal menyimpan role user: ' . $error_msg];
				}
			}
		}
		...
		$save = false;
		if ($trans) {
			// Set save = true karena transaction berhasil
			// Avatar update adalah opsional, tidak boleh membuat save = false
			$save = true;
			...
			// Update avatar hanya jika ada perubahan
			if ($new_name !== ($img_db['avatar'] ?? '')) {
				$data_db = [];
		...
		if ($save) {
			...
			// Jika transaction gagal, ambil error dari database
			$error = $this->db->error();
	...
	public function uploadExcel()
	{
		...
		$path = ROOTPATH . 'jdih/writable/temp/';
		...
		$message = [];
		$message['ok'] = [
			'Data berhasil di masukkan ke dalam tabel user sebanyak ' . format_ribuan($row_inserted) . ' baris',
	...
	public function deleteUser()
	{
		...
		if ($trans) {
			if (!empty($user['avatar'])) {
				delete_file(ROOTPATH . 'user/' . $user['avatar']);
			}
		} else {
			return false;
		}
	...
	public function updatePassword()
	{
		...
		$update = $this->db->query(
			'UPDATE user SET password = ? 
									WHERE id_user = ? ',
			[$password_hash, $this->user['id_user']]
		);
		return $update;
	}
}
