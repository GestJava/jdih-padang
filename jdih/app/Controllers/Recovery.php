...
class Recovery extends \App\Controllers\BaseController
{
		...
		if (!$error) {
			...
			if ($dbtoken) {
				...
				if ($dbtoken['expires'] < date('Y-m-d H:i:s')) {
					$message['message'] = 'Link expired, silakan request <a href="' . base_url('/recovery') . '">link reset password</a> yang baru';
					$error = true;
				} else if (!$auth->validateToken($url_token, $dbtoken['token'])) {
					$message['message'] = 'Token invalid, silakan request <a href="' . base_url('/recovery') . '">link reset password</a> yang baru';
					$error = true;
				}
			} else {
				$message['message'] = 'Token tidak ditemukan, silakan request <a href="' . base_url('/recovery') . '">link reset password</a> yang baru';
				$error = true;
			}
		}
		...
		if (!$error) {
			if (!empty($_POST['submit'])) {
				...
				if (!$error) {
					...
					if ($update) {
						$message['status'] = 'ok';
						$message['message'] = 'Password Anda berhasil diupdate, sekarang Anda dapat <a href="' . base_url('login') . '">Login</a> menggunakan password baru Anda';
					} else {
	...
	private function validateForm()
	{
		...
		// Only run CSRF validation if auto_check is enabled
		$config = config('App');
		if ($config->csrf['enable'] && $config->csrf['auto_check']) {
