...
class Register extends \App\Controllers\BaseController
{
	...
	public function __construct()
	{
		...
		$this->addJs(base_url('vendors/jquery/jquery.min.js'));
		$this->addJs(base_url('vendors/bootstrap/js/bootstrap.min.js'));

		$this->addStyle(base_url('vendors/bootstrap/css/bootstrap.min.css'));
		$this->addStyle(base_url('vendors/font-awesome/css/font-awesome.min.css'));
		$this->addStyle(base_url('themes/modern/css/register.css'));

		$this->addJs(base_url('vendors/jquery.pwstrength.bootstrap/pwstrength-bootstrap.min.js'));
		$this->addJs(base_url('themes/modern/js/password-meter.js'));
	}
	...
	public function confirm()
	{
		...
		if (!$error) {
			...
			if ($dbtoken) {
				...
				if ($user['verified'] == 1) {
					...
					$message['message'] = 'Link expired, silakan request <a href="' . base_url('register/resendlink') . '">link aktivasi</a> yang baru';
					$error = true;
				} else if (!$auth->validateToken($url_token, $dbtoken['token'])) {
					$message['message'] = 'Token invalid, silakan <a href="' . base_url('register') . '">register</a> ulang atau request <a href="' . base_url('resendlink') . '">link aktivasi</a> yang baru';
					$error = true;
				}
			} else {
		...
		if (!$error) {
			...
			if ($update) {
				$message['status'] = 'ok';
				$message['message'] = 'Selamat!!!, akun Anda berhasil diaktifkan, Anda sekarang dapat <a href="' . base_url('login') . '">Login</a> menggunakan akun Anda';
			} else {
	...
	private function validateForm()
	{
		...
		// Cek email
		$user_exists = $this->model->getUserByEmail($_POST['email']);
		if ($user_exists) {
			if ($user_exists['verified'] == 0) {
				$error['message'] = 'Email sudah terdaftar tetap belum diaktifkan, silakan <a href="' . base_url('register/resendlink') . '" title="Kirim ulang link aktivasi">aktifkan disini</a>';
			}
			return $error;
		}
	...
	private function validateFormResendlink()
	{
		...
		if ($validate) {
			...
			if ($user) {
				if ($user['verified'] == 1) {
					$error[] = 'Akun sudah pernah diaktifkan, silakan <a href="' . base_url('login') . '" title="Login">login</a> ke akun Anda';
				}
			} else {
