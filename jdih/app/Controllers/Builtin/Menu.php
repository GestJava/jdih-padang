...
class Menu extends \App\Controllers\BaseController
{
	...
	public function __construct() {
		...
		$this->addStyle ( base_url('vendors/jquery-nestable/jquery.nestable.min.css?r=').time());
		$this->addStyle ( base_url('vendors/wdi/wdi-modal.css?r=') . time());
		$this->addStyle ( base_url('vendors/wdi/wdi-fapicker.css?r=') . time());
		$this->addStyle ( base_url('vendors/wdi/wdi-loader.css?r=') . time());
		
		$this->addJs ( base_url('vendors/wdi/wdi-fapicker.js?r=') . time());
		$this->addJs (base_url('themes/modern/builtin/js/admin-menu.js'));
		$this->addJs ( base_url('vendors/jquery-nestable/jquery.nestable.js?r=') . time());
		$this->addJs ( base_url('vendors/js-yaml/js-yaml.min.js?r=') . time());
		$this->addJs ( base_url('vendors/jquery-nestable/jquery.wdi-menueditor.js?r=') . time());
		...
		$this->addStyle ( base_url('vendors/jquery.select2/bootstrap-5-theme/select2-bootstrap-5-theme.min.css') );
		
		$this->addJs(base_url('vendors/dragula/dragula.min.js'));
		$this->addStyle(base_url('vendors/dragula/dragula.min.css'));
	...
	public function index()
	{
		...
		if ($data['menu_kategori']) {
			$id_menu_kategori = $data['menu_kategori'][0]['id_menu_kategori'];
		} else {
			$id_menu_kategori = null;
		}
		$result = $this->model->getMenuByKategori($id_menu_kategori);
	...
	public function editMenu()
	{
		...
		if (isset($_POST['nama_menu'])) 
		{
			...
			if ($error) {
				...
				$data['msg']['message'] = '<ul class="list-error"><li>' . join('</li><li>', $error) . '</li></ul>';
			} else {
