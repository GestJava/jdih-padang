...
class Setting_layout extends \App\Controllers\BaseController
{
	public function __construct()
	{
		...
		$this->addJs(base_url('themes/modern/builtin/js/setting-layout.js?r=') . time());
		$this->addStyle(base_url('themes/modern/builtin/css/setting-layout.css'));
	...
	public function index()
	{
		...
		$bootswatch_path = ROOTPATH . '../vendors/bootswatch';
		if (is_dir($bootswatch_path)) {
			$themes = scandir($bootswatch_path);
			// Filter out . and .. and only include directories
			$data['list_bootswatch_theme'] = array_filter($themes, function ($item) use ($bootswatch_path) {
				return $item !== '.' && $item !== '..' && is_dir($bootswatch_path . '/' . $item);
			});
		} else {
			$data['list_bootswatch_theme'] = [];
		}
