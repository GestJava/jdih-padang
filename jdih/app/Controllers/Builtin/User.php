...
class User extends \App\Controllers\BaseController
{
	...
	public function __construct()
	{
		...
		$this->addJs(base_url('vendors/bootstrap-datepicker/js/bootstrap-datepicker.min.js'));
		$this->addJs(base_url('themes/modern/js/date-picker.js'));
		$this->addJs(base_url('themes/modern/builtin/js/user.js'));
		$this->addJs(base_url('themes/modern/builtin/js/image-upload.js'));
		$this->addStyle(base_url('vendors/bootstrap-datepicker/css/bootstrap-datepicker.min.css'));
	...
	public function getDataDT()
	{
		...
		foreach ($user['data'] as $key => &$val) {
			...
			$val['ignore_avatar'] = '<img src="' . base_url('public/user/') . $avatar . '">';
			...
			if ($this->hasPermission('delete_own') || $this->hasPermission('delete_all')) {
				$btn['delete'] = [
					'url' => $this->moduleURL,
					'id' =>  $val['id_user'],
					'delete-title' => 'Hapus data user: <strong>' . (strlen($val['nama']) > 30 ? substr($val['nama'], 0, 27) . '...' : $val['nama']) . '</strong> ?'
				];
			}
			$val['ignore_btn_action'] = btn_action($btn);

			// Batasi karakter nama untuk tampilan tabel
			$val['nama'] = (strlen($val['nama']) > 25) ? substr($val['nama'], 0, 22) . '...' : $val['nama'];
		}
	...
	public function setData()
	{
		...
		$this->data['list_instansi'] = $this->model->getListInstansi();
	}
