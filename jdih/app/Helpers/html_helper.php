	...
	foreach ($data as $key => $val) {
		if ($key == 'edit') {
			...
			foreach ($val['attr'] as $attr_name => $attr_value) {
				...
				$attr .= $attr_name . '="' . esc($attr_value, 'attr') . '"';
			}
			...
			$html .= '<form method="post" action="' . $data[$key]['url'] . '">
					<button type="submit" data-action="delete-data" data-delete-title="' . esc($data[$key]['delete-title'], 'attr') . '" class="btn btn-danger btn-xs">
						<span class="btn-label-icon"><i class="fa fa-times pe-1"></i></span> Delete
					</button>
					<input type="hidden" name="delete" value="delete"/>
					<input type="hidden" name="id" value="' . esc($data[$key]['id'], 'attr') . '"/>
					' . csrf_field() . '
				</form>';
		} else if ($key == 'proses') {
			...
			$html .= '<a href="' . $data[$key]['url'] . '" ' . $attr . '>
					<span class="btn-label-icon"><i class="fa fa-cogs pe-1"></i></span> Proses
				</a>';
		} else {
			$attr = '';
			if (key_exists('attr', $data[$key])) {
				foreach ($data[$key]['attr'] as $key_attr => $val_attr) {
					$attr .= $key_attr . '="' . esc($val_attr, 'attr') . '"';
				}
			}

			$html .= '<a href="' . $data[$key]['url'] . '" class="btn ' . esc($data[$key]['btn_class'], 'attr') . ' btn-xs me-1" ' . $attr . '>
						<span class="btn-label-icon"><i class="' . esc($data[$key]['icon'], 'attr') . '"></i></span>&nbsp;' . esc($data[$key]['text']) . '
					</a>';
		}
	}
