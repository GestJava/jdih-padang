...
$csrf_settoken = false;
function csrf_settoken($force = false) 
{
	...
	// CRITICAL SECURITY FIX: Allow force regenerate untuk login/logout
	if (!$force && $csrf_settoken) {
		return false;
	} 
	// Cek apakah url yang diakses adalah static resource, jika ya maka skip
	else if (!$force && strpos($current_url, 'public/') !== false) { 
		return false;
	}
	
	// CRITICAL SECURITY FIX: Generate token unik menggunakan random_bytes
	// Hapus cookie lama sebelum membuat yang baru
	// CRITICAL FIX: Cookie secure flag harus sesuai dengan HTTPS detection
	// Jika HTTPS, gunakan secure=true, jika tidak gunakan secure=false
	$isHttps = false;
	if (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on') {
		...
		$isHttps = true;
	} elseif (isset($_SERVER['HTTP_HOST']) && strpos($_SERVER['HTTP_HOST'], 'jdih.padang.go.id') !== false) {
		$isHttps = true;
	}
	
	setcookie($config->csrf['name'], '', time() - 360000, '/', '', $isHttps, true);
	$token = generate_token(32); // Generate 32 bytes = 64 hex characters
	setcookie($config->csrf['name'], $token, time() + $config->csrf['expire'], '/', '', $isHttps, true);
	session()->set('csrf_token', ['token' => $token]);
	$csrf_settoken = true;
	
	// Log untuk audit (hanya di development/debug mode)
	if (ENVIRONMENT !== 'production') {
		log_message('debug', 'CSRF token generated: ' . substr($token, 0, 8) . '...');
	}
}
