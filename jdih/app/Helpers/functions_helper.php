...
/**
Functions Helper
Original: https://webdev.id
JDIH Kota Padang Implementation
 */
...
if (!function_exists('set_value')) {
	/**
	 * Custom set_value dengan escape untuk keamanan XSS
	 * FIX: Tambahkan escape untuk mencegah XSS
	 */
	function set_value($field_name, $default = '', $escape = true)
	{
		...
		if (!empty($_POST[$field_name])) {
			if ($is_array) {
				...
				$index = $exp_close[0];
				$value = $_POST[$field_name][$index] ?? $default;
			} else {
				$value = $_POST[$field_name];
			}
		}

		// FIX: Escape untuk mencegah XSS
...
function upload_file($path, $file)
{
	...
	if (!empty($errors)) {
		log_message('error', 'File upload validation failed: ' . implode(', ', $errors));
		return false;
	}
	...
	// Move uploaded file
	$move = move_uploaded_file($file['tmp_name'], $path . $safe_name);

	if ($move) {
		// Set proper permissions
		chmod($path . $safe_name, 0644);
		return $safe_name;
	} else {
		log_message('error', 'Failed to move uploaded file: ' . $file['name']);
		return false;
	}
}
