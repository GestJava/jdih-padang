...
<div class="verifikasi-premium-shell container-fluid">
                ...
                <div class="card-body p-0">
                    <div class="row g-0 align-items-center">
                        <div class="col-lg-8 p-5 welcome-text">
                            <h6 class="text-uppercase ls-2 text-white-50 fw-bold mb-2">Portal Validasi</h6>
                            <h1 class="display-5 fw-800 text-white mb-3"><?= esc($title) ?></h1>
                            <p class="lead text-white-50 mb-4">Lakukan penelaahan substansi dan harmonisasi akhir sebelum dokumen diputuskan sebagai produk hukum final.</p>
                            <div class="d-flex gap-2">
                                <span class="badge glass-badge px-3 py-2 rounded-pill border border-white border-opacity-10 text-white">
                                    <i class="material-icons align-middle fs-6 me-1">fact_check</i> <?= count($ajuan_list) ?> Ajuan Menunggu Validasi
                                </span>
                            </div>
                        </div>
                        <div class="col-lg-4 d-none d-lg-block text-center p-4">
                            <div class="banner-visual-shell">
                                <i class="material-icons banner-icon">verified</i>
    ...
    <!-- Validator Workload (Horizontal Glass Scroll) -->
    <?php if (!empty($validator_stats)): ?>
    <div class="row mb-5">
        <div class="col-12">
            <h5 class="section-title mb-4 fw-800 text-dark">
                <i class="material-icons align-middle me-2 text-primary">groups</i> Beban Kerja Validator
            </h5>
            <div class="row g-4">
                <?php 
                $gradients = [
                ...
                foreach (array_slice($validator_stats, 0, 6) as $index => $stat): 
                ...
                <div class="stat-card card border-0 shadow-sm glass-bg-emerald h-100">
                    <div class="card-body p-4">
                        ...
                        <div class="stat-content">
                            ...
                            <h6 class="text-white-50 text-uppercase ls-1 fw-bold small">Validasi Selesai</h6>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card border-0 shadow-premium rounded-4 overflow-hidden">
                <div class="card-header bg-white py-4 px-4 border-0 d-flex align-items-center justify-content-between">
                    <div>
                        <h5 class="fw-800 text-dark mb-1">Ajuan Menunggu Validasi</h5>
                        <p class="text-muted small mb-0">Total <?= count($ajuan_list) ?> dokumen sedang dalam antrean validasi</p>
                    </div>
                    <div class="table-actions">
                        <span class="badge bg-success bg-opacity-10 text-success px-3 py-2 rounded-pill fw-bold">
                            Scope: <?= $data_scope ?? 'Validasi' ?>
                        </span>
                    </div>
                </div>
                <div class="card-body p-0">
                                ...
                                <?php if (!empty($ajuan_list)): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <?php if ($ajuan_id): ?>
                                                <a href="<?= base_url('validasi/proses/' . esc($ajuan_id)) ?>" 
                                    ...
                                    <tr>
                                        <td colspan="6" class="text-center py-5">
                                            <div class="empty-state">
                                                ...
                                                <h5 class="text-muted mt-3">Tidak ada ajuan yang perlu divalidasi.</h5>
                                            </div>
                                        </td>
                                    </tr>
                                <?php endif; ?>
...
<style>
    /* Premium Design System for Validasi Page */
    :root {
    ...
    .banner-gradient { position: absolute; top: 0; left: 0; right: 0; bottom: 0; background: linear-gradient(135deg, #059669 0%, #064e3b 100%); z-index: 1; }
        ...
        if ($.fn.DataTable && $('#data-tables').length) {
            var table = $('#data-tables').DataTable({
                ...
                language: {
                    ...
                    emptyTable: 'Tidak ada dokumen dalam antrean validasi saat ini.'
                },
                order: [[3, 'desc']],
                columnDefs: [
